...
def app_servers():
    ...
    frontend_proc = subprocess.Popen(
        [sys.executable, "-m", "streamlit", "run", "frontend/streamlit_app.py", "--server.port", "8501", "--server.address", "127.0.0.1"],
        env=env,
        stdout=frontend_log,
        stderr=frontend_log,
    )
    ...
    while time.time() - start_time < 60:  # Allow up to 60 seconds for models to load and server to start
        # Check backend
...
def test_e2e_happy_path(app_servers, page, tmp_path):
    ...
    doc.add_paragraph("Senior Python Developer at Tech Corp. Developed and scaled backend APIs using FastAPI and Python.")
...
""")
    ...
    assert len(screenshots) > 0, f"Failure screenshot not created! Pytest Output: {res.stdout}\n{res.stderr}"
