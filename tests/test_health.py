...
@pytest.mark.parametrize(
    ...
    [
        ...
        "https://ekmfboeemgmecad7bfufbb.streamlit.app",
    ],
)
def test_cors_allows_streamlit_browser_origin(client, origin):
...
def test_build_empty_timestamp_uses_unknown_fallback(client, monkeypatch):
...
def test_version_fallback_behavior(client, monkeypatch):
    ...
    assert response.status_code == 200
    body = response.json()
    assert body["version"] == "1.0.0"  # fallback default
