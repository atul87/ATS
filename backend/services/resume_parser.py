...
from backend.core.config import MAX_FILE_SIZE_BYTES, MAX_FILE_SIZE_MB, SUPPORTED_MIME_TYPES
...
def validate_file(file_data: bytes, filename: str) -> Tuple[bool, str, Optional[str]]:
    ...
    if mime_type not in SUPPORTED_MIME_TYPES:
        ...
        return (
            False,
            (f"Unsupported file type: {mime_type}. " f"Please upload one of: {supported}."),
            None,
        )
...
def _extract_pdf_with_pdfplumber(file_data: bytes) -> str:
    ...
    if not text.strip():
        raise TextExtractionError(
            "pdfplumber extracted no text", user_message="No text could be extracted from the PDF."
        )
...
def _extract_pdf_with_pypdf(file_data: bytes) -> str:
    ...
    if not text.strip():
        raise TextExtractionError(
            "pypdf extracted no text", user_message="No text could be extracted from the PDF."
        )
...
def extract_text_from_pdf(file_data: bytes) -> str:
    try:
        result, used_fallback = with_fallback(
            _extract_pdf_with_pdfplumber, _extract_pdf_with_pypdf, file_data, log_fallback=True
        )
