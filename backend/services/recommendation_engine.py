...
def generate_formatting_recommendations(
    ...
    core_missing = [
        (n, s) for n, s in missing_sections if n in ["experience", "education", "skills"]
    ]
...
def _prioritize_recommendations(recommendations: List[Recommendation]) -> List[Recommendation]:
    ...
    return sorted(recommendations, key=lambda r: (priority_order[r.priority], -r.impact_score))
