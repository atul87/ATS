...
# Configure page
st.set_page_config(
    page_title="ATS Resume Scorer", page_icon="🎯", layout="wide", initial_sidebar_state="expanded"
)
...
# Check if mock/offline mode is active and not running under E2E tests
is_e2e = os.getenv("E2E_TESTING", "").lower() == "true" or "e2e" in st.query_params
is_offline = os.getenv("MOCK_AUTH", "true").lower() in {"1", "true", "yes", "on"}

if is_offline and not is_e2e:
    # Auto-login in offline/mock mode if the user hasn't explicitly logged out in this session
    if not st.session_state.access_token and not st.session_state.get("explicit_logout", False):
        st.session_state.access_token = "mock-access-token"
        st.session_state.refresh_token = "mock-refresh-token"
        st.session_state.user_id = "mock-user-id"
        st.session_state.user_email = "local-user@example.com"
...
with st.sidebar:
    ...
    if is_offline and not is_e2e:
        st.success("🟢 Local Offline Mode Active")
        st.caption("All data is saved locally on your laptop.")
